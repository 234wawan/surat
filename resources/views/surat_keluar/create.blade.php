<x-app-layout>
            ...
            <form action="{{ route('surat-keluar.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <select id="master_surat_id" name="master_surat_id" class="mt-1.5 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-lg shadow-sm">
                            ...
                            @foreach($masterSurat as $ms)
                                <option value="{{ $ms->id }}" {{ old('master_surat_id') == $ms->id ? 'selected' : '' }}>{{ $ms->kode }} — {{ $ms->nama }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mb-5">
                    ...
                    <div class="mt-1.5 flex items-center gap-3">
                        <x-text-input id="no_surat" name="no_surat" type="text" class="block w-full" :value="old('no_surat')" />
                        <x-text-input id="no_urut" name="no_urut" type="number" min="1" class="w-24 shrink-0" placeholder="No urut" :value="old('no_urut')" title="Isi untuk skip/lompat nomor urut" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                    <div>
                        ...
                        <x-text-input id="tujuan" name="tujuan" type="text" class="mt-1.5 block w-full" :value="old('tujuan')" required />
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function previewNomor() {
            ...
            if (!msId || !tgl) {
                noSurat.value = '';
                previewInfo.textContent = 'Nomor surat akan digenerate otomatis setelah memilih jenis surat dan tanggal.';
        ...
        if (masterSurat.value && tanggalSurat.value) previewNomor();
    });
    </script>
    @endpush
</x-app-layout>
