            ...
            <form action="{{ route('role.update', $role) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1.5 block w-full" :value="old('name', $role->name)" required />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="slug" value="Slug (ID Unik)" />
                        <x-text-input id="slug" name="slug" type="text" class="mt-1.5 block w-full" :value="old('slug', $role->slug)" required />
    ...
    <script>
        function roleForm() {
            return {
                ...
                init() {
                    const existingMenus = @json($roleMenus);
                    this.allMenus.forEach(menu => {
                        this.menus[menu.id] = existingMenus.includes(menu.id);
                        menu.children.forEach(child => {
                            this.menus[child.id] = existingMenus.includes(child.id);
                        });
                    });
