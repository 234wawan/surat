<x-app-layout>
    <x-slot name="header">Tambah Master Surat</x-slot>

    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <form action="{{ route('master-surat.store') }}" method="POST">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                    <div>
                        <x-input-label for="kode" value="Kode" />
                        <x-text-input id="kode" name="kode" type="text" class="mt-1.5 block w-full" :value="old('kode')" required placeholder="Contoh: PERDIR" />
                        <x-input-error :messages="$errors->get('kode')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="nama" value="Nama Surat" />
                        <x-text-input id="nama" name="nama" type="text" class="mt-1.5 block w-full" :value="old('nama')" required placeholder="Surat Perintah Direktur" />
                ...
                <div class="mb-5">
                    ...
                    <x-text-input id="pola_nomor" name="pola_nomor" type="text" class="mt-1.5 block w-full" :value="old('pola_nomor')" placeholder="{no}/PDAM/{tahun}" />
